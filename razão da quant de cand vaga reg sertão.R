...
# 1. Filtrar e calcular a razão por município — somente Sertão
razao_sertao <- database_candidatos_candidatas %>%
  filter(str_to_lower(reg) == "sertão") %>%
  group_by(municipio) %>%
  summarise(
...
menores <- razao_sertao %>%
  arrange(candidata_vaga) %>%
  slice_head(n = 10) %>%
  mutate(grupo = "Menores razões")

maiores <- razao_sertao %>%
  arrange(desc(candidata_vaga)) %>%
  slice_head(n = 10) %>%
  mutate(grupo = "Maiores razões")

extremos_sertao <- bind_rows(menores, maiores)

# 3. Gráfico de linhas com legenda para Sertão
ggplot(extremos_sertao, aes(x = reorder(municipio, candidata_vaga), y = candidata_vaga, group = grupo, color = grupo)) +
