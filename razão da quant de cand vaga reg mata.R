...
# 1. Filtrar e calcular a razão por município — somente Mata
razao_mata <- database_candidatos_candidatas %>%
  filter(str_to_lower(reg) == "mata") %>%
  group_by(municipio) %>%
  summarise(
...
extremos_mata <- bind_rows(menores, maiores)

# 3. Gráfico de linhas com legenda para Mata
ggplot(extremos_mata, aes(x = reorder(municipio, candidata_vaga), y = candidata_vaga, group = grupo, color = grupo)) +
