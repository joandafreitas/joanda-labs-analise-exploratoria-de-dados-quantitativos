...
# 1. Filtrar e calcular a razão por município — somente Agreste
razao_agreste <- database_candidatos_candidatas %>%
  filter(str_to_lower(reg) == "agreste") %>%
  group_by(municipio) %>%
  summarise(
...
menores <- razao_agreste %>%
  arrange(candidata_vaga) %>%
  slice_head(n = 10) %>%
  mutate(grupo = "Menores razões")

maiores <- razao_agreste %>%
  arrange(desc(candidata_vaga)) %>%
  slice_head(n = 10) %>%
  mutate(grupo = "Maiores razões")

extremos_agreste <- bind_rows(menores, maiores)

# 3. Gráfico de linhas com legenda
ggplot(extremos_agreste, aes(x = reorder(municipio, candidata_vaga), y = candidata_vaga, group = grupo, color = grupo)) +
